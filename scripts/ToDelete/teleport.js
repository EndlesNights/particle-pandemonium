...
export const handleTeleportRequestGM = async (data) => {
  ...
  if (userId === game.userId) {
    ...
    await game.socket.emit('module.particleEmitters', { eventName: 'tokenSelectRequestPlayer', data })
  }
}
...
export const handleTokenSelectRequestPlayer = async (data) => {
  ...
  // pan to particleEmitter target
  canvas.pan({ x: targetData.x, y: targetData.y })

  // call hook
  Hooks.callAll('ParticleEmitterTeleport', data)
}
